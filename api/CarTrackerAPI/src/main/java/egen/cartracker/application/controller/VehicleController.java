...
public class VehicleController {
	...
	public ResponseEntity<List<Vehicle>> retrievePartnersAvailability(@RequestBody List<Vehicle> requestBody) {
		...
		try {
			...
			for (Vehicle v : requestBody)
			{
				
				Vehicle vehicle = vehicleRepository.findOne(v.getVin());
				if (vehicle == null) {partnerList.add(v); }
				else{
					vehicle.setYear(v.getYear());
					vehicle.setRedlineRpm(v.getRedlineRpm());
					vehicle.setModel(v.getModel());
					vehicle.setMaxFuelVolume(v.getMaxFuelVolume());
					vehicle.setMake(v.getMake());
					vehicle.setLastServiceDate(v.getLastServiceDate());
	...
	public Map<String, Object> readAll() {
		...
		dataMap.put("totalVehicles", vehicles.size());
